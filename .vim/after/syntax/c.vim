...
"" vim: ts=8
syn keyword	cConstant	__LINE__ __FILE__ __DATE__ __TIME__ __STDC__
syn keyword	cType		VOID UCHAR USHORT ULONG UING ULONGLONG WORD DWORD void
syn keyword	cType		PVOID PUCHAR PUSHORT PULONG PULONGLONG PWORD PDWORD UINT8 UINT64 UINT16 UINT32
syn keyword	cType		BOOL BOOLEAN NTSTATUS
syn keyword	cType		URB IRP PURB PIRP 
syn keyword	cType		NDIS_STATUS
syn keyword	cType		HANDLE FILE 
syn keyword	cBoolean	TRUE true FALSE false
syn keyword	cConditional	__try __finally __leave
...
" #define 后面的字符串被认为时宏定义
syn match	cMacro		/#define\s\+\zs[a-zA-Z0-9_]\+/
...
syn match	cMemberVariable	/\.\s*\zs[a-zA-Z0-9_]\+/
syn match	cMemberVariable	/->\s*\zs[a-zA-Z0-9_]\+/
"syn match	cVariable	/[a-zA-Z0-9_]\+->/
"syn match	cMacro		/\<[A-Z0-9_]\+\>/


hi def link	cFunction	Function
hi def link	cBoolean	Boolean
hi def link	cMemberVariable	Conditional
hi def link	cMacro		Macro
hi def link	cLogicalOperator Operator

"use TAB instead of space, only for SSD project in Micron (Tidal).
"set noexpandtab
